...
echo -e "${BRed}✖ ${white}Uso correcto: ./sattack.sh [-i <address>] [-p <port>] [-t <0-5>] [-m (Machine) <true|false>] [-s minecraft|other]"
echo -e "${white}i = Address | p = Port | t = Time Policy | m = Specificy Machine | s = Service"
echo -e "\n${white}Ejemplo: ./sattack.sh -i 164.132.119.73 -p 25565 -t 5 -m false -s minecraft"
echo -ne "\n${white}———————————————————————————————————————————\n"
exit 1
}
...
function ctrl_c(){
echo " "
echo -e "${red} Cerrando script.."; exit 0
}
...
while getopts ":i:p:t:m:s:" o; do
	case "${o}" in
		...
		t)
		  t=${OPTARG}
		  ((t == 0 || t == 1 || t == 2 || t == 3 || t == 4 || t == 5)) || helpUsage
		  ;;
		m)
		  m=${OPTARG}
...
echo -e "${white} ◆ ${red}Tiempo de policy: ${white}${t}"
echo -e "${white} ◆ ${red}Detectar Maquina: ${white}${m}"
echo -e "${white} ◆ ${red}Servicio: ${white}${s}"
echo -e "\n${white}▶———————————————————————————————————————————◀"
sleep 5; echo -e "\n\t${green}Escaneando puertos.."
sleep 5
if [ ${m} = "true" ]; then
...
clear
sleep 2; echo -e "\n${BGreen}✔ ${white}Se han escaneado los puertos correctamente. Leyendo resultados.."
sleep 3
echo -ne "\n${green}▶———————————————————————————————————————————◀${white}\n"
cat ~/Desktop/target.txt
echo -ne "\n${green}▶———————————————————————————————————————————◀\n"
sleep 5; echo -e "\n\t${purple}Cerrando script en 10 segundos.."
