...
echo -e "${red} Servicio: ${gray}${a}"
echo " "
echo -e "${purple}---------------------------${clear}"
echo -e "${green} Escaneando puertos del objetivo.."

if [ ${a} = 'minecraft' ]; then
	touch ~/Desktop/target.txt; sleep 3; nmap -oN ~/Desktop/target.txt -p 2000-65535 -Pn -v -A ${i} &>/dev/null
elif [ ${a} = 'machine' ]; then
...
echo -e "${gray}Obteniendo informacion del archivo ${yellow}target.txt ${gray}que se encuentra en el escritorio.."
echo " "
sleep 4
echo -e "${green}---------------------------------${gray}"
